    ...
    <div class="table">
      ...
      <table>
        ...
        <?php
          ...
          $result = mysql_query("SELECT * FROM parentreg NATURAL JOIN login WHERE parentreg.userid=login.userid AND type='parent' AND status='0'");
          ...
          if($no>0)
          {
            ...
            <td><?php 
                  $id=$row['stid'];
                  $sq=mysql_query("SELECT * FROM studentreg WHERE stid='$id'");
                  while ($s=mysql_fetch_array($sq))
                  {
                    echo $s['stfname'].' '.$s['stlname'];
                  }
                  ?></td>
