<?php
  ...
  else 
  {
    ...
    if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $newfilename)) 
      {  
         $sq=mysql_query("UPDATE notice SET `date`='$date'WHERE nid='$id'");
         echo "<script>alert('Notice Updated')</script>";
        echo"<script>location.href='uploadnotice.php';</script>";
      } 
      else 
      {
...
if($_GET['task']=='del')
  {
    ...
    $datetime=explode(" ",$timestamp);
    $odate=$datetime[0];
    unlink('../uploads/notice/'.$id.' '.$description.'('.$odate.')'.".".'pdf');
