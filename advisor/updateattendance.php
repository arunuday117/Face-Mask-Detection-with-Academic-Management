<?php
  ...
  if(isset($_POST['add']))
  {
    ...
    $result = mysql_query("SELECT * FROM attendance WHERE atid='$id'");
    ...
    $msg3='';
    $msg4='';
    $filename=$target_dir .$id.' '.$sem.' '.$batch.' '.$description.'('.$odate.')'.".".$FileType;
    $newfilename=$target_dir .$id.' '.$sem.' '.$batch.' '.$description.'('.$dateq.')'.".".$FileType;
  // Check file size
  ...
  else 
  {
    ...
    if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $newfilename)) 
      {  
        $sq=mysql_query("UPDATE attendance SET `date`='$date'WHERE atid='$id'");
...
if($_GET['task']=='del')
  {
    ...
    $datetime=explode(" ",$timestamp);
    $odate=$datetime[0];
    unlink('../uploads/attendance/'.$id.' '.$sem.' '.$batch.' '.$description.'('.$odate.')'.".".'pdf');
     $sq="DELETE FROM attendance WHERE atid='$id' ";//Data Deletion code
 ...
 ?>
  ...
  <div class="main">
    <div class="course">
      ...
      <?php
        $id=$_GET['id'];
        $sq=mysql_query("SELECT * FROM attendance WHERE atid='$id'");
      ...
      <form action="" method="post" id="course" enctype="multipart/form-data">
        ...
        <input type="text" name="sem" readonly value="<?php echo $ro['sem'];?>" id="name">
