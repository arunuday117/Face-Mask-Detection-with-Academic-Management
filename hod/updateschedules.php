<?php
  ...
  if(isset($_POST['add']))
  {
    ...
    $result = mysql_query("SELECT * FROM exam WHERE eid='$id'");
    ...
    $msg3='';
    $msg4='';
    $filename=$target_dir .$id.' '.$description.'('.$odate.')'.".".$FileType;
    $newfilename=$target_dir .$id.' '.$description.'('.$dateq.')'.".".$FileType;
  // Check file size
  ...
  else 
  {
    ...
    if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $newfilename)) 
      {  
        $sq=mysql_query("UPDATE exam SET `date`='$date'WHERE eid='$id'");
        echo "<script>alert('Exam Schedule Updated')</script>";
        echo"<script>location.href='uploadschedules.php';</script>";
      } 
      else 
      {
...
if($_GET['task']=='del')
  {
    ...
    $datetime=explode(" ",$timestamp);
    $odate=$datetime[0];
    unlink('../uploads/schedules/'.$id.' '.$description.'('.$odate.')'.".".'pdf');
     $sq="DELETE FROM exam WHERE eid='$id' ";//Data Deletion code
    if(mysql_query($sq,$con))
    {
      echo"<script>alert('Exam Schedule Deleted!!');</script>";
      echo"<script>location.href='uploadschedules.php';</script>";
    }
  }
?>
...
<body>
 ...
 ?>
  ...
  <div class="main">
    <div class="course">
      <div class="heading">
        <h6>Update Exam Schedules</h6>
      </div>
      <?php
        $id=$_GET['id'];
        $sq=mysql_query("SELECT * FROM exam WHERE eid='$id'");
      ...
      <form action="" method="post" id="course" enctype="multipart/form-data">
        ...
        <input type="text" readonly name="description"id="name"value="<?php echo $ro['description'];?>">
        ...
        <input type="text" name="type" readonly value="<?php echo $ro['batch'];?>" id="name">
