...
{
    $sql="select max(pid) as pid from parentreg";
    ...
    if(empty($stid))
    {
        $error_msg['stid']="**University Registration Number is required";
        $flag=1;
    }
    if(empty($first_name))
    {
    ...
    if($flag!=1)
    {	
        if (!preg_match("/^\d{11}$/", $stid))
        {
            $error_msg['stid']="*Invalid University Registration Number";
            $flag=1;
        }
    ...
    if($flag==0)
    {
        ...
        $data="SELECT * FROM parentreg natural join login";
        $c=0;
        $sd="SELECT * FROM parentreg";
        ...
        {
            if($row['userid']==$email||$row['pphone']==$phone)
            {
                $flag=1;
            }
        }
        ...
        {
            $sql="INSERT INTO `parentreg`VALUES ('$pid','$first_name','$last_name','$email','$housename','$street','$district','$state','$pin','$phone','$stid')";
            if(mysql_query($sql,$con))
            {
                $sql1="INSERT INTO `login`VALUES ('$email','$pass','parent',0,'','')";
          ...
          <div class="right-content">
            ...
            <form id="contact" action="" method="post">
              <div class="row">
                ...
                <div class="col-md-12">
                  <fieldset>
                    <input name="stid" type="text" class="form-control" id="stid" placeholder="Enter Your Ward's University Registration Number"value="<?php if(isset($stid)){echo $stid;}  ?>"required>
                  </fieldset>
                </div>
                <?php
