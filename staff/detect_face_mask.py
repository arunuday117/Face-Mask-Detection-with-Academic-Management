        ...
        if(len(mouth_rects) == 0):
            ...
            for (mx, my, mw, mh) in mouth_rects:
                ...
                if(y < my < y + h):
                    ...
                    if os.path.isfile('violation/image'+str(i)+'.png'):
                        i+=1
                        continue
                    else:
                        resp = requests.post('http://localhost/ANV ACADEMICS/staff/check.php?id=nomask',params={"img":i})
                        print(resp.content)
                        cv2.imwrite('violation/image'+str(i)+'.png',img)
